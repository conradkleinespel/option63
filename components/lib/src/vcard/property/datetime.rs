...
type DT<'a, T> = RE<'a, T, DateAndOrTimeOrTextError>;

impl<'a> nom::error::ParseError<&'a [u8]> for DateAndOrTimeOrTextError {
    fn from_error_kind(_input: &'a [u8], _kind: nom::error::ErrorKind) -> Self {
        DateAndOrTimeOrTextError::InvalidFormat
    }
...
pub enum DateAndOrTimeOrTextError {
    ...
    InvalidFormat,
    #[error("date/time component out of bounds")]
    OutOfBounds,
...
fn digits_n<const N: usize, T>(input: &[u8]) -> DT<'_, T>
where
    T: std::str::FromStr,
{
...
fn parse_date_and_or_time_for_v40(s: &[u8]) -> DT<'_, DateAndOrTimeOrText> {
...
fn parse_date_time_for_v40(s: &[u8]) -> DT<'_, DateTime> {
...
fn parse_date_time_iso8601_basic(s: &[u8]) -> DT<'_, DateTime> {
...
// "T"
fn parse_time_designator(s: &[u8]) -> DT<'_, ()> {
    tag("T").parse_complete(s).map(|(rest, _)| (rest, ()))
}

fn parse_date_time_iso8601_extended(s: &[u8]) -> DT<'_, DateTime> {
...
fn parse_date_iso8601_extended(s: &[u8]) -> DT<'_, Date> {
    ...
    if !valid_month(month) || !valid_day(day) {
        return Err(nom::Err::Error(DateAndOrTimeOrTextError::OutOfBounds));
    }
    Ok((rest, Date::new(Some(year), Some(month), Some(day))))
}

fn parse_time_iso8601_extended(s: &[u8]) -> DT<'_, TimeWithZone> {
...
// year
fn parse_date_year_for_v40(s: &[u8]) -> DT<'_, Date> {
    let (rest, year) = digits_n::<4, i32>(s)?;
    Ok((rest, Date::new(Some(year), None, None)))
}

// year "-" month
fn parse_date_year_month_for_v40(s: &[u8]) -> DT<'_, Date> {
    let (rest, year) = digits_n::<4, i32>(s)?;
    let (rest, _) = tag("-").parse_complete(rest)?;
    let (rest, month) = digits_n::<2, u32>(rest)?;
...
fn parse_date_iso8601_basic(s: &[u8]) -> DT<'_, Date> {
    let (rest, year) = digits_n::<4, i32>(s)?;
    let (rest, month) = digits_n::<2, u32>(rest)?;
    let (rest, day) = digits_n::<2, u32>(rest)?;
    if !valid_month(month) || !valid_day(day) {
...
// "--"     month
fn parse_date_month_for_v40(s: &[u8]) -> DT<'_, Date> {
...
// "--"     month day
fn parse_date_month_day_for_v40(s: &[u8]) -> DT<'_, Date> {
    let (rest, _) = tag("--").parse_complete(s)?;
    let (rest, month) = digits_n::<2, u32>(rest)?;
    let (rest, day) = digits_n::<2, u32>(rest)?;
    if !valid_month(month) || !valid_day(day) {
...
// "--"      "-"   day
fn parse_date_day_for_v40(s: &[u8]) -> DT<'_, Date> {
    let (rest, _) = tag("--").parse_complete(s)?;
    let (rest, _) = tag("-").parse_complete(rest)?;
    let (rest, day) = digits_n::<2, u32>(rest)?;
    if !valid_day(day) {
...
fn parse_date_for_v40(s: &[u8]) -> DT<'_, Date> {
    alt((
...
fn parse_date_noreduc_for_v40(s: &[u8]) -> DT<'_, Date> {
    alt((
        parse_date_iso8601_basic,
        parse_date_month_day_for_v40,
        parse_date_day_for_v40,
...
// 2DIGIT
fn parse_time_hour_for_v40(s: &[u8]) -> DT<'_, Time> {
    let (rest, hour) = digits_n::<2, u32>(s)?;
    if hour > 23 {
        return Err(nom::Err::Error(DateAndOrTimeOrTextError::OutOfBounds));
    }
...
// 2DIGIT 2DIGIT
fn parse_time_hour_minute_for_v40(s: &[u8]) -> DT<'_, Time> {
    let (rest, hour) = digits_n::<2, u32>(s)?;
    let (rest, minute) = digits_n::<2, u32>(rest)?;
...
// hour minute second
fn parse_time_hour_minute_second(s: &[u8]) -> DT<'_, Time> {
    let (rest, hour) = digits_n::<2, u32>(s)?;
    let (rest, minute) = digits_n::<2, u32>(rest)?;
    let (rest, second) = digits_n::<2, u32>(rest)?;
...
// "-"  minute second
fn parse_time_minute_second_for_v40(s: &[u8]) -> DT<'_, Time> {
    let (rest, _) = tag("-").parse_complete(s)?;
    let (rest, minute) = digits_n::<2, u32>(rest)?;
    let (rest, second) = digits_n::<2, u32>(rest)?;
...
// "-" minute
fn parse_time_minute_for_v40(s: &[u8]) -> DT<'_, Time> {
    let (rest, _) = tag("-").parse_complete(s)?;
    let (rest, minute) = digits_n::<2, u32>(rest)?;
    if minute > 59 {
        return Err(nom::Err::Error(DateAndOrTimeOrTextError::OutOfBounds));
...
// "-"   "-"    second
fn parse_time_second_for_v40(s: &[u8]) -> DT<'_, Time> {
    let (rest, _) = tag("--").parse_complete(s)?;
    let (rest, second) = digits_n::<2, u32>(rest)?;
    if second > 59 {
        return Err(nom::Err::Error(DateAndOrTimeOrTextError::OutOfBounds));
...
fn parse_time_for_v40(s: &[u8]) -> DT<'_, TimeWithZone> {
    let (rest, time) = alt((
...
fn parse_time_notrunc_for_v40(s: &[u8]) -> DT<'_, TimeWithZone> {
    let (rest, time) = alt((
        parse_time_hour_minute_second,
        parse_time_hour_minute_for_v40,
        parse_time_hour_for_v40,
...
fn parse_zone_utc(s: &[u8]) -> DT<'_, i32> {
    tag("Z").parse_complete(s).map(|(rest, _)| (rest, 0))
}

// sign digit digit ":" digit digit
fn parse_zone_sign_hh_mm(s: &[u8]) -> DT<'_, i32> {
...
fn parse_zone_iso8601_extended(s: &[u8]) -> DT<'_, Option<i32>> {
...
fn parse_zone_iso8601_basic(s: &[u8]) -> DT<'_, Option<i32>> {
    if let Ok((rest, zone)) = parse_zone_utc(s) {
        return Ok((rest, Some(zone)));
    }
...
fn zone_two_digits(input: &[u8]) -> DT<'_, i32> {
    digits_n::<2, i32>(input)
}

// ( "+" / "-" ) hour [minute]
fn parse_zone_utc_offset_iso8601_basic(s: &[u8]) -> DT<'_, i32> {
    let (rest, sign) = one_of("+-").parse_complete(s)?;
    let (rest, hour) = digits_n::<2, i32>(rest)?;
